...
function _broadcast(was_broadcast_callback) {
    return new Promise((resolve, reject) => {
        ...
        const params = !!was_broadcast_callback? [
            function(res) {
                return resolve(res);
            },
            tr_object
        ...
        Apis.instance()
            .network_api()
            .exec(!!was_broadcast_callback? "broadcast_transaction_with_callback": "broadcast_transaction", params)
            .then(function() {
                //console.log('... broadcast success, waiting for callback')
                if (was_broadcast_callback) was_broadcast_callback(params[0]);
                return;
            })
            .catch(error => {
                //console.log('... broadcast failture, delete sub')
                delete Apis.instance().network_api().ws_rpc.subs[params[0]]; 
