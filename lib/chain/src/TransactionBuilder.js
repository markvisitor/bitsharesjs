...
class TransactionBuilder {
    constructor(tx = null) {
        ...
        this._broadcast2 = _broadcast2.bind(this);
    }
    ...
    */
    ...
    //was_broadcast_callback is null , then call broadcast_transaction
    //was_broadcast_callback is not null, then call broadcast_transaction_with_callback
    /**see useage: 
     broadcast2(function(res){
        //transaction success 
     }).then(()=>{
        //broadcast success
     })
     **/
    broadcast2(was_broadcast_callback) {
        if (this.tr_buffer) {
            return this._broadcast2(was_broadcast_callback);
        } else {
            return this.finalize().then(() => {
                return this._broadcast2(was_broadcast_callback);
            });
...
function _broadcast2(was_broadcast_callback) {
    return new Promise((resolve, reject) => {
        if (!this.signed) {
            this.sign();
        }
        ...
        var tr_object = ops.signed_transaction.toObject(this);
        const params = [tr_object];
        let method = 'broadcast_transaction';
        if(!!was_broadcast_callback) {
            //broadcast success, waiting for callback
            params.unshift(was_broadcast_callback)
